<template>
  <div class="app-container">
    ...
    <header>
      <h1>{{ appTitle }}</h1>
      <p>亚健康诊断 · 科学膳食配比 · 作息恢复与运动复健 · 健康避坑警示</p>
    </header>
    ...
    <main ref="inputCardRef" class="glass-card input-group">
      ...
      <div class="selector-group">
        <label class="selector-label">选择养生与健康调理类型</label>
        <div class="style-selector">
          <button 
            v-for="htype in healthTypeOptions" 
            :key="htype"
            class="style-option"
            :class="{ active: activeHealthType === htype }"
            @click="activeHealthType = htype"
          >
            {{ htype }}
          </button>
        </div>
      ...
      <div class="options-row" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
        <div class="selector-group">
          <label class="selector-label">目标人群</label>
          <div class="style-selector">
            <button 
              v-for="group in targetGroupOptions" 
              :key="group"
              class="style-option"
              :class="{ active: selectedTargetGroup === group }"
              @click="selectedTargetGroup = group"
            >
              {{ group }}
            </button>
          </div>
        ...
        <div class="selector-group">
          <label class="selector-label">关注重点</label>
          <div class="style-selector">
            <button 
              v-for="focus in healthFocusOptions" 
              :key="focus"
              class="style-option"
              :class="{ active: selectedHealthFocus === focus }"
              @click="selectedHealthFocus = focus"
            >
              {{ focus }}
            </button>
          </div>
      ...
      <div class="selector-group">
        <div style="display: flex; justify-content: space-between; align-items: center;">
          <label class="selector-label">输入您的健康状况、不适症状或生活习惯</label>
          <div style="display: flex; gap: 0.5rem;">
            <button v-if="userInput" class="text-link-btn" @click="userInput = ''">清空输入</button>
            <button class="text-link-btn" @click="showHealthRulesModal = true">养生误区与避坑红线指南</button>
          </div>
        </div>
        <textarea 
          v-model="userInput" 
          placeholder="请简要描述您的身体状况与需求...（例如：程序员经常加班久坐，最近感觉脖子僵硬、腰背酸痛，晚上经常失眠多梦，希望制定一份拉伸复健与助眠安神方案。）"
          style="min-height: 120px;"
        ></textarea>
        <div style="display: flex; justify-content: space-between; font-size: 0.75rem; color: var(--text-secondary);">
          <span>字符数: {{ userInput.length }} 字</span>
          <span>建议明确年龄段、职业生活习惯及具体不适感受</span>
        </div>
      </div>
      ...
      <div style="display: flex; gap: 0.75rem;">
        ...
        >
          {{ loading ? '正在精准评估症状与设计科学调理方案中...' : '开始生成科学养生与健康照护方案' }}
        </button>
    ...
    <section v-if="result || loading" class="glass-card">
      <div class="result-header">
        <span class="result-title">科学养生与健康照护方案</span>
      ...
      <div v-else-if="result">
        ...
        <div v-if="aiScores" class="scores-container" style="margin-bottom: 1.5rem; padding: 1.25rem; background: rgba(0,0,0,0.25); border-radius: 12px; border: 1px solid rgba(255,255,255,0.06);">
          <div style="font-weight: 700; font-size: 0.95rem; margin-bottom: 1rem; color: #a5b4fc; display: flex; justify-content: space-between; align-items: center;">
            <span>AI 科学养生与健康评估看板</span>
            <span style="font-size: 0.8rem; font-weight: normal; color: var(--text-secondary);">综合健康指导分: {{ getAverageScoreFromMap(aiScores) }} / 100</span>
          </div>
    ...
    <section v-if="showHistory" class="glass-card" style="margin-top: 1rem;">
      <div class="result-header">
        <span class="result-title">本地健康养生历史记录</span>
      ...
      <div v-else class="history-grid" style="display: flex; flex-direction: column; gap: 0.75rem; max-height: 320px; overflow-y: auto;">
        <div v-for="item in historyList" :key="item.id" class="history-item" style="padding: 1rem; background: rgba(0,0,0,0.2); border-radius: 10px; border: 1px solid var(--card-border);">
          <div style="display: flex; justify-content: space-between; font-size: 0.8rem; color: var(--text-secondary); margin-bottom: 0.4rem;">
            <span>{{ item.timestamp }} · [{{ item.healthType }} / {{ item.targetGroup }} / {{ item.healthFocus }}]</span>
    ...
    <!-- 科学养生误区与健康避坑红线指南 Modal -->
    <div v-if="showHealthRulesModal" class="modal-overlay" @click.self="showHealthRulesModal = false">
      <div class="modal-content" style="max-width: 480px;">
        <h3>科学养生误区与健康避坑红线指南</h3>
        <p style="text-align: left; font-size: 0.825rem; margin-bottom: 1rem; color: var(--text-secondary);">
          遵守科学规程、避免盲信偏方与延误就医的核心指导原则：
        </p>
        <div class="modal-scroll-area" style="text-align: left; font-size: 0.825rem;">
          <div v-for="(rule, idx) in healthRules" :key="idx" style="margin-bottom: 0.75rem; padding: 0.5rem 0.75rem; background: rgba(255,255,255,0.03); border-radius: 8px; border: 1px solid rgba(255,255,255,0.05);">
            <div style="color: var(--accent-color); font-weight: bold; margin-bottom: 0.2rem;">{{ rule.title }}</div>
            <div style="color: var(--text-primary); margin-bottom: 0.2rem;">科学建议: {{ rule.advice }}</div>
            <div style="color: var(--text-secondary); font-size: 0.775rem;">避坑红线: {{ rule.avoid }}</div>
          </div>
        </div>
        <button class="modal-btn" style="margin-top: 1rem;" @click="showHealthRulesModal = false">关闭</button>
      </div>
    </div>
    ...
    <div v-if="showShareGuide" class="modal-overlay" @click.self="showShareGuide = false">
      <div class="modal-content">
        <h3>分享科学养生与健康照护专家</h3>
        <p>扫码关注或将链接分享给亲朋好友，共同打造健康生活习惯。</p>
    ...
    <div v-if="showPrivacy" class="modal-overlay" @click.self="showPrivacy = false">
      <div class="modal-content">
        ...
        <div class="modal-text-content modal-scroll-area">
          <p>我们非常重视您的个人健康数据与隐私保护。您在本应用中输入的症状描述、体征参数与健康疑问仅用于实时大模型生成，系统不会在云端永久存储或公开泄露您的个人隐私数据。</p>
    ...
    <div v-if="showTerms" class="modal-overlay" @click.self="showTerms = false">
      <div class="modal-content">
        ...
        <div class="modal-text-content modal-scroll-area">
          <p>欢迎使用网腾无限 AI 科学养生与健康照护专家。本工具生成的膳食营养建议、作息复健计划及养生知识仅供日常生活参考，不能代替专业医师诊疗或处方建议。</p>
          <p>若您感到急性身体不适或存在严重慢性疾病，请务必及时到正规医疗机构就诊。</p>
        </div>
...
// 配置参数
const appTitle = ref(appConfig.title || '网腾无限AI - 科学养生与健康照护专家');
...
const showHealthRulesModal = ref(false);
...
const activeHealthType = ref(healthTypeOptions[0]);

// 2 组属性：目标人群 & 关注重点
const targetGroupOptions = ['职场白领', '产后妈妈', '中老年长辈', '健身爱好者'];
const selectedTargetGroup = ref('职场白领');

const healthFocusOptions = ['提升免疫力', '改善睡眠', '护眼明目', '消化肠道调理'];
const selectedHealthFocus = ref('提升免疫力');

// 评估指标列表
const metricsList = [
  { key: 'medicalScientificness', label: '医学原理科学度' },
  { key: 'dietaryNutritionRigor', label: '膳食营养严谨度' },
  { key: 'subhealthInsight', label: '亚健康调理契合度' },
  { key: 'lifestyleFeasibility', label: '生活方式可操作性' },
  { key: 'wellnessSafetyMargin', label: '养生防坑安全度' }
];
...
const healthRules = [
  { 
    title: '盲目补品与偏方误区', 
    advice: '遵循营养均衡原则，优先通过日常膳食补充维生素与膳食纤维，不盲从无根据偏方。', 
    avoid: '严禁将保健食品、偏方神药当做万能药替代正规临床医学诊断与治疗。' 
  },
  { 
    title: '过度节食与极端排毒', 
    advice: '维持三大营养素合理比例，科学控制卡路里，保持规律一日三餐与饮水。', 
    avoid: '切勿采取断食、绝食或过量服用泻药等伤害脾胃脏器功能的极端养生方式。' 
  },
  { 
    title: '剧烈运动与忽视体征预警', 
    advice: '根据个人体质与年龄制定循序渐进的运动复健计划，注意心率监控与运动后恢复。', 
    avoid: '身体出现严重胸闷、头晕、持续发热或急性剧痛时严禁强行运动，须即刻就医。' 
  }
];

// 计算纯结果文本 (剔除打分标签 [JIANKANG_SCORES])
const displayResultText = computed(() => {
  if (!result.value) return '';
  return result.value.replace(/\[JIANKANG_SCORES\][\s\S]*?\[\/JIANKANG_SCORES\]/g, '').trim();
});

// 解析打分标签
const parseAiScores = (rawText: string) => {
  const match = rawText.match(/\[JIANKANG_SCORES\](.*?)\[\/JIANKANG_SCORES\]/);
...
const loadHistory = () => {
  try {
    const raw = localStorage.getItem('jiankang_history_records');
...
const saveHistory = () => {
  localStorage.setItem('jiankang_history_records', JSON.stringify(historyList.value));
};
...
const addHistoryRecord = () => {
  const newItem: HistoryItem = {
    ...
    healthType: activeHealthType.value,
    targetGroup: selectedTargetGroup.value,
    healthFocus: selectedHealthFocus.value,
...
const applyHistory = (item: HistoryItem) => {
  ...
  activeHealthType.value = item.healthType;
  if (item.targetGroup) selectedTargetGroup.value = item.targetGroup;
  if (item.healthFocus) selectedHealthFocus.value = item.healthFocus;
...
const handleGenerate = async () => {
  ...
  try {
    const response = await fetch(apiEndpoint, {
      ...
      body: JSON.stringify({
        taskType: 'text',
        prompt: `任务指导: ${promptTopic.value}\n【健康与养生调理类型】: ${activeHealthType.value}\n【目标人群】: ${selectedTargetGroup.value}\n【关注重点】: ${selectedHealthFocus.value}\n【身体状况与需求描述】: ${userInput.value}`,
        style: activeHealthType.value
      })
    });
...
const handleApplyTemplate = (payload: {
  ...
  if (payload.healthType) activeHealthType.value = payload.healthType;
  if (payload.targetGroup) selectedTargetGroup.value = payload.targetGroup;
  if (payload.healthFocus) selectedHealthFocus.value = payload.healthFocus;
  if (inputCardRef.value) {
    inputCardRef.value.scrollIntoView({ behavior: 'smooth', block: 'center' });
  }
};
